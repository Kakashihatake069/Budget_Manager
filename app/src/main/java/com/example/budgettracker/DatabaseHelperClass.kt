...
class DatabaseHelperClass(var context: Context) : SQLiteOpenHelper(context, "CategoryTb", null, 1) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        db?.execSQL(sql)
        var sqll ="create table StorageTb(storage_id integer primary key autoincrement,type interger,amount integer,categoryselect text,adddate text,addtime text,selectmode text,note text)"
        db?.execSQL(sqll)
    }
    ...
    fun displayincomeExpencedata(): ArrayList<Valuemodel> {
        ...
        if(cursor.count>0)
        {
            ...
            do {
                ...
                val modeltwo = Valuemodel(id,type,amount,categoryselect,adddate,addtime,selectmode,note)
                datastorage.add(modeltwo)


                Log.e("TAG", "display: "+ type +" "+amount+" "+categoryselect+" "+adddate+" "+addtime+" "+selectmode+" "+note)
    ...
    fun updaterecord(amount: String, note: String, storage_id: Int) {
        ...
        val sql1 = "update StorageTb set amount='$amount',note='$note' where storage_id ='$storage_id'"
        db.execSQL(sql1)
